...
class Stage1VideoDataset(Dataset):
    def __init__(self, samples: list[dict], *, frames: int, size: int, spatial=None,
                 temporal=None) -> None:
        ...
        self.views = _temporal_eval_views(self.temporal)

    def __len__(self) -> int:
        return len(self.samples) * len(self.views)

    def __getitem__(self, index: int) -> tuple[torch.Tensor, int, str]:
        video_index, view_index = divmod(index, len(self.views))
        path = self.samples[video_index]["path"]
...
def main() -> None:
    ...
    dataset = Stage1VideoDataset(
        samples, frames=frames, size=size, spatial=spatial, temporal=temporal,
    )
    loader = DataLoader(
        dataset,
    ...
    probabilities: list[list[float]] = [[] for _ in samples]
    view_probabilities: list[dict[str, float]] = [{} for _ in samples]
    with torch.inference_mode():
        for clips, indices, views in tqdm(loader, desc="evaluate"):
            ...
            for index, view, probability in zip(
                indices.tolist(), views, rerecorded_probabilities.cpu().tolist()
            ):
                value = float(probability)
                probabilities[index].append(value)
                view_probabilities[index][view] = value
    ...
    targets = []
    predictions = []
    for sample, probability_values, per_view in zip(
        samples, probabilities, view_probabilities
    ):
        probability = float(sum(probability_values) / len(probability_values))
        ...
        if len(per_view) > 1:
            for view, value in per_view.items():
                row[f"{view.replace('-', '_')}_probability"] = value
        rows.append(row)
    ...
    print(f"temporal sampling: {temporal}")
